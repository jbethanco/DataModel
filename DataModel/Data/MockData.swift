...
enum MockData{
    ...
    //provide a persistance controller for SwiftUI previews
    ...
    static let crewLine     = MockData.crewLines.randomElement()!
    static let mission      = MockData.missions.randomElement()!
    static let person       = MockData.persons.randomElement()!
    static let sortie       = MockData.sorties.randomElement()!

    //provide arrays of CoreData entities for SwiftUI Previews
    static var crewLines:   [CrewLine]  = { MockData.getEntities(name: "CrewLine")  as! [CrewLine]  }()
    static var missions:    [Mission]   = { MockData.getEntities(name: "Mission")   as! [Mission]   }()
    static let persons:     [Person]    = { MockData.getEntities(name: "Person")    as! [Person]    }()
    static let sorties:     [Sortie]    = { MockData.getEntities(name: "Sortie")    as! [Sortie]    }()
  
    //used above to get arrays of any entity by name from the database
    static func getEntities(name named: String) -> [Any] {
        let controller = MockData.preview
    ...
    static let missionNames: [String] = {
        var names = [String]()
    ...
    static let missionSummaries: [String] = {
    ...
    static func loadMosherForm(viewContext: NSManagedObjectContext = PersistenceController.shared.container.viewContext){
...
//        let viewContext = persistenceController.container.viewContext
        ...
        mission.name = "Exercise Scorpion"
        mission.summary = "Drop USAF personnel at North Auxillary Airfield and return."
    ...
    static func loadMockData1(viewContext: NSManagedObjectContext = PersistenceController.shared.container.viewContext){
//        let persistenceController = PersistenceController.shared
//        let viewContext = persistenceController.container.viewContext
        var personCounter = 0
        for i in 0...5 {
            let mission = Mission(context: viewContext)
            //newItem.name = Date()
            mission.name = MockData.missionNames[i]
            mission.summary = MockData.missionSummaries[i]
            
            for j in 0..<3 {
                let sortie = Sortie(context: viewContext)
                sortie.mission = mission
                sortie.takeoffICAO = MockData.icaos[j + i]
                sortie.landICAO = MockData.icaos[j + i + 1]
                sortie.takeoffTime = MockData.takeOffAndLandTimes[j + i].takeoff
                sortie.landTime = MockData.takeOffAndLandTimes[j + i].land
                sortie.mds = MockData.mds[i]
                if i == 0 {
                    //the first Mission has all different serial numbers so we can test splitting the Form 781 on different aircraft
                    sortie.serialNumber = MockData.serialNumbers[j]
                }else{
                    sortie.serialNumber = MockData.serialNumbers[i]
                }
                sortie.harmLocation = MockData.harmLocations[i]
                sortie.unitCharged = MockData.unitCharged[i]
                
                for c in 0..<(3 + i) {
                    let person = Person(context: viewContext)
                    person.firstName = MockData.firstNames[personCounter]
                    person.lastName = MockData.lastNames[personCounter]
                    personCounter += 1
                    
                    let crewLine = CrewLine(context: viewContext)
                    crewLine.flightAuthDutyCode = MockData.flightAuthDutyCodes[c]
...
extension UIWindow {
    open override func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        if motion == .motionShake {
            MockData.loadMosherForm()
            MockData.loadMockData1()
        }
    }
}
#endif
